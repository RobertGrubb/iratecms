...
$base_url= str_replace('/plugins/filemanager', '', 'http'.(isset($_SERVER['HTTPS'])?'s':'').'://'.$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF']));  // base url (only domain) of site (without final /). If you prefer relative urls leave empty
$upload_dir = '/uploads/source/'; // path from base_url to base of upload folder (with start and final /)
$current_path = '../../uploads/source/'; // relative path from filemanager folder to upload folder (with final /)
...
// With Responsive file manager youcayn create automaticall resized image inside the upload folder, also more than one at a time
